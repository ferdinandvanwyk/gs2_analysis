...
def get_field_3d(cdf_file, nc_var_name, time_idx, spec_idx):
    """
    Read field from ncfile and prepare for calculations.
    ...
    * Apply fourier correction
    ...
    nc_file = Dataset(cdf_file, 'r')
    nc_var = np.array(nc_file.variables[nc_var_name][time_idx,spec_idx,:,:,:,:])
...
def field_to_real_space_3d(field):
    """
    Converts field from (kx, ky) to (x, y) and saves as new array attribute.
    """
    shape = field.shape
